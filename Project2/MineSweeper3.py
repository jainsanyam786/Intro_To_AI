...
class MineSweeper3(object):
    ...
    def giveprobability(self):
        ...
        # if the agent is "Improved Probabilistic (IP)
        if self.agent == "IP":
            solArray = self.validsolution(solArray)
        if result:      # if solutions exist for the equations
            ...
            # for all the variables in the result
...
def main(cls):
    ...
    if "analysis".casefold().__eq__(Mode.casefold()):
        ...
        sizes = [30, 40, 50, 60]
        mdenisty = 0.40
        # (P:Probabilistic / IP:Improved Probabilistic)
        agent = "IP"
        iterations = 5
